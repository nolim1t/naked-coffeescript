	...
	request {uri: url, method: 'GET'}, (error, response, body) ->
		if not error and response.statusCode == 200
			...
			if res_dict.access_token != 'error'
				...
				request {uri: conf.foursquare.baseUrl + 'users/self?oauth_token=' + encodeURIComponent(access_token), method: 'GET'}, (aterr, atres, atbody) ->
...
showtrending = (ll, callback) ->
	url = conf.foursquare.baseUrl + 'venues/trending?ll=' + encodeURIComponent(ll) + '&client_id=' + encodeURIComponent(conf.foursquare.accessToken) + '&client_secret=' + encodeURIComponent(conf.foursquare.accessSecret)
	request {uri: url, method: 'GET'}, (error, response, body) ->
		if not error and response.statusCode == 200
...
showcategories = (callback) ->
	url = conf.foursquare.baseUrl + 'venues/categories?client_id=' + encodeURIComponent(conf.foursquare.accessToken) + '&client_secret=' + encodeURIComponent(conf.foursquare.accessSecret)
	request {uri: url, method: 'GET'}, (error, response, body) ->
		if not error and response.statusCode == 200
			res_dict = JSON.parse(body)['response']
			category_list = []
...
searchvenue = (name, address, callback) ->
	url = conf.foursquare.baseUrl + 'venues/search?query=' + encodeURIComponent(name) + '&near=' + encodeURIComponent(address) + '&radius=1000&client_id=' + encodeURIComponent(conf.foursquare.accessToken) + '&client_secret=' + encodeURIComponent(conf.foursquare.accessSecret)
	request {uri: url, method: 'GET'}, (error, response, body) ->
		if not error and response.statusCode == 200
			res_dict = JSON.parse(body)['response']
			places = []
...
matchvenue = (name, address, callback) ->
	url = conf.foursquare.baseUrl + 'venues/search?intent=match&query=' + encodeURIComponent(name) + '&near=' + encodeURIComponent(address) + '&radius=1000&client_id=' + encodeURIComponent(conf.foursquare.accessToken) + '&client_secret=' + encodeURIComponent(conf.foursquare.accessSecret)
	request {uri: url, method: 'GET'}, (error, response, body) ->
		if not error and response.statusCode == 200
			res_dict = JSON.parse(body)['response']
			matchedplaces = []
